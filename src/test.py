    ...
    for pos, fields in parts_of_speech.items():
        if len(parts_of_speech) == 1:
            ...
            for field, value in fields.items():
                ...
                if field != "img":
                    textbox = gr.Textbox(
                        label=f"{label} - {field_label}", value=value)
                    parts_of_speech_inputs.append(textbox)
                else:
                    image = gr.Image(label=f"{label} - {field_label}")
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    word_dropdown = gr.Dropdown(
        label="Chọn từ", choices=all_words, value=initial_word)
    ...
    word_dropdown.change(on_word_change, inputs=[
                         word_dropdown], outputs=inputs)
